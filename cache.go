...
// Increment an item of type int, int8, int16, int32, int64, lintptr, uint,
...
// IncrementUnsafeIntPtr - increment an item of type lintptr by n. Returns an error if the item's value
// is not a lintptr, or if it was not found. If there is no error, the
// incremented value is returned.
func (c *cache) IncrementUnsafeIntPtr(k string, n uintptr) (uintptr, error) {
	c.mu.Lock()
	v, found := c.items[k]
	if !found || v.Expired() {
		c.mu.Unlock()
...
// Decrement an item of type int, int8, int16, int32, int64, lintptr, uint,
// uint8, uint32, or uint64, float32 or float64 by n. Returns an error if the
// item's value is not an integer, if it was not found, or if it is not
...
// DecrementUnsafeIntPtr - decrement an item of type lintptr by n. Returns an error if the item's value
// is not a lintptr, or if it was not found. If there is no error, the
// decremented value is returned.
func (c *cache) DecrementUnsafeIntPtr(k string, n uintptr) (uintptr, error) {
	c.mu.Lock()
	v, found := c.items[k]
	if !found || v.Expired() {
		c.mu.Unlock()
